...
class DataVisApp:
    ...
    def setup_ui(self):
        ...
        self.plots_container_wrapper.pack(fill=tk.BOTH, expand=True, pady=10)
        ...
        # Add Plot button
        add_plot_btn = ttk.Button(self.left_panel, text="+ Add Subplot", command=self.add_plot)
        add_plot_btn.pack(fill=tk.X, pady=5)
        
        # Draw Plot button
        draw_btn = ttk.Button(self.left_panel, text="3. Draw / Update Plots", command=self.draw_plots)
    ...
    def draw_plots(self):
        ...
        for ax, cols in zip(axes, plot_configs):
            for col in cols:
                ax.plot(time_data, filtered_df[col], label=col)
