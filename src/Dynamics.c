...
// this funnction predicts apogee of the rocket given current altitude, velocity, and control input


// neeed to improve this to take into account when the airbrakes will actually be at given deployment (lag)
float PredictApogee(float h_sim_predict, float v_sim_predict, float u) {
 ...
 float dt = (predict_dt - predict_dt_final)/(h_burnout - h_final) * (h_sim_predict - h_burnout) + predict_dt;
 dt = MIN(predict_dt, MAX(dt, predict_dt_final));


 //dt   // slightly bigger for speed - smaller more accurate - doesnt have to be same as control loop
...
// this functions finds the control input that minimising the error between predicted apogee and target apogee
// need to update to use either gradient descent or binary search for faster computation time
float OptimiseControlInput(float h, float v, float u_prev) {
 ...
 for (int i = 0; i < iterations; i++) {
   ...
   float objectiveVal = alpha * (href - apogee)*(href - apogee) + (1.0f - alpha) * (u - u_prev)*(u - u_prev);
...
float OptimiseControlInputBinarySearch(float h, float v, float u_prev) {
 ...
 while(1) {
   ...
   float u = (u_max - u_min)/((float)optimiser_array_length - 1.0f) * (float)m;
...
// this optimiser does the same as above but takes into acconut servo speed limits
float OptimiseControlInputBinarySearchConstraint(float h, float v, float u_prev, int iteration) {
 ...
 // debug print
 if (iteration == 0 || iteration == 1 || iteration == 2 || iteration == 3 || iteration == 4 || iteration == 5 || iteration == 6) {
   Serial.printf("udt_max: %.3f ,uprev = %.3f, u_min = %.3f, umax = %.3f\n",udt_max, u_prev, u_constrained_min, u_constrained_max);
 }
 int count = 0;
 ...
 while(1) {
   ...
   float u = (u_range/((float)(length))) * (float)(m - length) + u_constrained_max;
...
// uses both accelerometer data and time since launch - can place weightings on each
// - should use interrupt
int CoastDetected(int launch_time) {
   // need to update to use accelerometer
 int time_since_launch = esp_timer_get_time() - launch_time;
 if (time_since_launch > (float)((burn_time + active_time_offset)*1000000.0f)) {
   return 1;
 } else {
   return 0;
 }
...
// need to log altitude, velocity, orientation, control input, predicted apogee, drag coefficent to sd card - ill do later
