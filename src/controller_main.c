...
void app_main(void) {
  ...
  float h_pred_initial = PredictApogee(state.h,state.v,0);
    ...
    while (1) {
        ...
        if (control_flag) {
            ...
            // ---- simulate dynamics ----
            rk4(&state.h, &state.v, 0.0f, GetCd(u), m_final, loop_dt);  // replace with sensor add some point
            ...
            // ---- debug ----
            // if (iteration == 0 || iteration == 1 || iteration == 2 || iteration == 3|| iteration == 4|| iteration == 5|| iteration == 6|| iteration == 7|| iteration == 8|| iteration == 9|| iteration == 10|| iteration == 11) {
        
            //  printf("h=%.3f v=%.2f u=%.2f dt_ms=%.3f\n", state.h, state.v, u, (float)(end_time - start_time)/1000.0f);
            // }
           ...
           optimise_time_history[iteration] = (float)(end_time - start_time)/1000.0f;
           loop_time_history[iteration] = (float)(loop_end_time - loop_start_time)/1000.0f;
  ...
  for (int i = 0; i < iteration; i++) {
    printf("t = %.3f, h=%.2f v=%.2f u=%.2f h_pred = %.2f optimise_time(ms) = %.3f loop time(ms) = %.3f\n", time_history[i]/1000000.f, altitude_history[i], velocity_history[i], control_input_history[i], predicted_apogee_history[i], optimise_time_history[i], loop_time_history[i]);
  }
}
