...
static void mat_mul(float C[EKF_N][EKF_N], float A[EKF_N][EKF_N],
                    float B[EKF_N][EKF_N]) {
  float tmp[EKF_N][EKF_N];
  mat_zero(tmp);
  for (int i = 0; i < EKF_N; i++)
    for (int j = 0; j < EKF_N; j++)
      for (int k = 0; k < EKF_N; k++) tmp[i][j] += A[i][k] * B[k][j];
  mat_copy(C, tmp);
}
...
static void mat_transpose(float T[EKF_N][EKF_N], float A[EKF_N][EKF_N]) {
  for (int i = 0; i < EKF_N; i++)
    for (int j = 0; j < EKF_N; j++) T[i][j] = A[j][i];
}

// C = A + B  (element-wise addition)
static void mat_add(float C[EKF_N][EKF_N], float A[EKF_N][EKF_N],
                    float B[EKF_N][EKF_N]) {
  for (int i = 0; i < EKF_N; i++)
    for (int j = 0; j < EKF_N; j++) C[i][j] = A[i][j] + B[i][j];
}
...
static void ekf_compute_noise_matrices(EKF* ekf) {
  ...
  float sigma_bgyro =
      GYRO_BIAS_INSTABILITY * (3.14159265358979323846f / 180.0f);
...
void ekf_update(EKF* ekf, float z_baro) {
  ...
  for (int i = 0; i < EKF_N; i++)
    for (int j = 0; j < EKF_N; j++) IKH[i][j] -= KH[i][j];
